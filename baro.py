...
# === NUEVAS FUENTES DE CONOCIMIENTO (APIs públicas como Wikipedia) ===
def search_wikidata(query):
    """Consulta Wikidata (base de conocimiento estructurado gratuito)"""
    query = query.strip()
    if not query:
        return ""
    try:
        ...
        if data.get("search"):
            ...
            return "\n".join(results)
        return "No encontrado en Wikidata."
    except Exception as e:
        print(f"Error Wikidata: {e}")
        return "Error al consultar Wikidata."

def search_dbpedia(query):
    """Consulta DBpedia (extracción estructurada de Wikipedia)"""
    query = query.strip()
    if not query:
        return ""
    try:
        # Construir título típico de recurso DBpedia
        title = "_".join(word.capitalize() for word in query.split())
        uri = f"http://dbpedia.org/resource/{title}"
        sparql = f"""
        ...
        r = requests.get("https://dbpedia.org/sparql", params=params, headers=headers, timeout=10)
        data = r.json()
        bindings = data.get("results", {}).get("bindings", [])
        if bindings:
            return bindings[0]["abstract"]["value"]
        # Fallback a inglés
        sparql_en = sparql.replace('langMatches(lang(?abstract), "es")', 'langMatches(lang(?abstract), "en")')
        params_en = {"query": sparql_en, "format": "application/sparql-results+json"}
        r_en = requests.get("https://dbpedia.org/sparql", params=params_en, headers=headers, timeout=10)
        data_en = r_en.json()
        bindings_en = data_en.get("results", {}).get("bindings", [])
        if bindings_en:
            return bindings_en[0]["abstract"]["value"] + " (en inglés)"
        return "No encontrado en DBpedia."
    except Exception as e:
        print(f"Error DBpedia: {e}")
        return "Error al consultar DBpedia."
...
def process_command(command):
    ...
    # === PREGUNTAS DE CONOCIMIENTO ===
        ...
        if topic:
            ...
            if local_info and score > 0.6:
                ...
                # Wikipedia primero
                wiki_response = search_wikipedia(topic)
                response = wiki_response
                
                # Si Wikipedia falla → buscar en Wikidata y DBpedia
                if "No encontré" in wiki_response or "No pude" in wiki_response or "error" in wiki_response.lower():
                    wd_response = search_wikidata(topic)
                    if "No encontrado" not in wd_response and "Error" not in wd_response:
                        response += "\n\nDesde Wikidata:\n" + wd_response
                    
                    dp_response = search_dbpedia(topic)
                    if "No encontrado" not in dp_response and "Error" not in dp_response:
                        response += "\n\nDesde DBpedia:\n" + dp_response
        else:
    ...
    # === BÚSQUEDA GENERAL DE CONOCIMIENTO ===
        ...
        if local_info and score > 0.5:
            ...
            # Si Wikipedia falla → buscar en Wikidata y DBpedia
            if "No encontré" in wiki_result or "No pude" in wiki_result or "error" in wiki_result.lower():
                wd_result = search_wikidata(command)
                if "No encontrado" not in wd_result and "Error" not in wd_result:
                    response += "\n\nDesde Wikidata:\n" + wd_result
                
                dp_result = search_dbpedia(command)
                if "No encontrado" not in dp_result and "Error" not in dp_result:
                    response += "\n\nDesde DBpedia:\n" + dp_result
                
                if "No encontré" in response and "Wikidata" not in response and "DBpedia" not in response:
                    response = "Lo siento, no encontré información sobre eso en mis fuentes disponibles."
    ...
    try:
        ...
        c.execute("INSERT INTO interactions (timestamp, command, response, intent, confidence) VALUES (?, ?, ?, ?, ?)",
                  (datetime.datetime.now().isoformat(), original_command, response, str(intent), confidence))
        conn.commit()
        conn.close()
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print("✅ Fuentes adicionales: Wikidata y DBpedia activadas")
    print("=" * 60)
